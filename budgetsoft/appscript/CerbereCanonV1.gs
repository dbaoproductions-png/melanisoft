const CERBERE_CANON_V1_VERSION = '1.0.0';

/**
 * Référentiel du budget canonique quotidien.
 * Il ne touche jamais aux opérations réelles. Les montants enregistrés ici sont
 * exclusivement prévisionnels et peuvent être réalloués par l'utilisateur.
 */
function chargerCanonCerbereV1() {
  verifierInitialisation_();
  const sh = assurerCanonCerbereV1_();
  const rows = lireTablePlanCerbere_('Cerbere_Canon_V1')
  ...
  return {
    ...
    postes:rows.map(r=>({
      ...
      monetaire:Number(r.monetaire||0),
      pluxee:Number(r.pluxee||0),
      nature:String(r.nature||'ajustable'),
      ordre:Number(r.ordre||99),
      protege:String(r.protege).toLowerCase()==='true'
    })),
...
function enregistrerCanonCerbereV1(postes) {
  ...
  const sh = assurerCanonCerbereV1_();
  const existants = lireTablePlanCerbere_('Cerbere_Canon_V1');
  const map = {};
  ...
  postes.forEach((p,i)=>{
    ...
    let mon=Math.max(0,Number(p.monetaire||0));
    let plu=Math.max(0,Number(p.pluxee||0));
    ...
    if(map[cat]) sh.getRange(map[cat].row,1,1,7).setValues([vals]);
...
function assurerCanonCerbereV1_(){
  ...
  // Canon V1 décidé ensemble. Projet est volontairement à 0 ici : l'interface
  // lui attribue le reliquat disponible après charges, enveloppes et épargne.
  const d=[
    ['Courses',656,94,'essentiel',1,false,true],
    ['Santé',50,0,'essentiel',2,false,true],
    ['Animaux',50,0,'ajustable',3,false,true],
    ['Maison / entretien',0,0,'ajustable',4,false,true],
    ['Voitures',95,0,'ajustable',5,false,true],
    ['Transports',39,0,'essentiel',6,false,true],
    ['Restaurants',79,60,'discretionnaire',7,false,true],
    ['Loisirs',100,0,'discretionnaire',8,false,true],
    ['Achats personnels',200,0,'discretionnaire',9,false,true],
    ['Épargne',50,0,'protection',10,true,true],
    ['Projet',0,0,'solde',11,false,true]
  ];
