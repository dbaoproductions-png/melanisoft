 ...
 */
const BUDGETSOFT_EP_20260913_VERSION='2026-09-13.1';
...
/**
 * Propriétaire EP d'un cycle. Les allocations x.prevu sont déjà la vue P0+molettes
 * construite par Cerbère V33 : aucun second stockage ni second moteur n'est créé.
 */
function calculerEnvelopePilotableBudgetSoft20260913_(p){
  ...
  return{
    ok:true,version:BUDGETSOFT_EP_20260913_VERSION,proprietaire:BUDGETSOFT_EP_OWNER_20260913,
    cle:String(p&&p.clePilotage||''),source:modifie?'molettes':'P0',p0:p0,total:ep,
    consomme:consomme,reste:arrEpBudgetSoft20260913_(ep-consomme),reservePlan:plan,lignes:lignes,
    doctrine:'EP = somme des allocations décidées ; sans modification des molettes, allocations=P0. EP ne modifie jamais P.'
  };
}

/** Attache la même vérité EP aux objets consommés par Cerbère/Dashboard/Express. */
function enrichirEnvelopePilotableBudgetSoft20260913_(base){
  ...
  ps.forEach((p,index)=>{
    ...
    c.pSoutenable=arrEpBudgetSoft20260913_(pSoutenable);c.pDisponible=pDisponible;
    c.ecartEpP=arrEpBudgetSoft20260913_(ep.total-pSoutenable);
    c.budgetRepartiMolettes=ep.total;
...
/**
 * Profil de paiement observé. Le 95 % n'est jamais une vérité codée en dur :
 * on le mesure sur les opérations pilotables ; 95 % n'est qu'un fallback si
...
function calculerProfilPaiementEpBudgetSoft20260913_(p,operations,reference){
  ...
  const ops0=Array.isArray(operations)?operations:(typeof lireTable_==='function'?(lireTable_('Operations')||[]):[]);
  ...
  let tauxCarte=echantillon.total>0?echantillon.carte/echantillon.total:.95;
  if(!Number.isFinite(tauxCarte)||tauxCarte<0||tauxCarte>1)tauxCarte=.95;
  const fallback=echantillon.total<100||echantillon.nombre<3;if(fallback)tauxCarte=.95;
...
/** Impact de la part non consommée de l'EP sur la trajectoire bancaire. */
