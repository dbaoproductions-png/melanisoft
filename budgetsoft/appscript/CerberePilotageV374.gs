...
function stabiliserCerbereV3717_(base){
  ...
  periodes.forEach((p,i)=>{
    ...
    Object.keys(remplacements).forEach(id=>{
      ...
      if(candidats.length!==1&&opsLiees.length)candidats=refLignes.filter(c=>!aliasUtilises.has(String(c&&c.id||''))&&opsLiees.some(o=>equivalentOperationCfReferenceV3721_(o,c)));
      if(candidats.length===1){
        ...
        ajustementsAlias.push({charge_fixe_id:id,reference_id:String(c&&c.id||''),prevu:arrV374_(prevu),reel:arrV374_(reel),delta:arrV374_(delta),source:'occurrence cycle'});
      }else if(maitre&&Math.abs(Number(maitre.montant||0))>0&&opsLiees.length){
        const prevu=Math.abs(Number(maitre.montant||0)),delta=reel-prevu;
        cft1AvantEffets+=delta;
        ajustementsAlias.push({charge_fixe_id:id,reference_id:'maitre:'+id,prevu:arrV374_(prevu),reel:arrV374_(reel),delta:arrV374_(delta),source:'montant maître de secours'});
      }else{
...
function equivalentOperationCfReferenceV3721_(op,ref){if(!op||!ref)return false;const ol=normaliserV377_(op.libelle_bancaire||op.libelle||''),rl=normaliserV377_(ref.libelle_bancaire||ref.libelle||'');if(!ol||!rl||Math.min(ol.length,rl.length)<4)return false;const nom=ol===rl||ol.indexOf(rl)>=0||rl.indexOf(ol)>=0;if(!nom)return false;const oc=normaliserV377_(op.categorie||''),rc=normaliserV377_(ref.categorie||'');return !oc||!rc||oc===rc||memeFamilleCfV3717_(oc,rc)||nom;}
function equivalentCfReferenceV3720_(maitre,ref){if(!maitre||!ref)return false;const ml=normaliserV377_(maitre.libelle_bancaire||maitre.libelle||''),rl=normaliserV377_(ref.libelle_bancaire||ref.libelle||'');const mc=normaliserV377_(maitre.categorie||''),rc=normaliserV377_(ref.categorie||'');const mm=Math.abs(Number(maitre.montant||0)),rm=Math.abs(Number(ref.montant||0));const nom=ml&&rl&&(ml===rl||ml.indexOf(rl)>=0||rl.indexOf(ml)>=0);const famille=memeFamilleCfV3717_(mc,rc);const montant=Math.abs(mm-rm)<=.011;return !!(nom||(famille&&montant));}
function estEffetCfEviteV3720_(x,charges,refLignes){if(!x||Number(x.montantSigne||0)<=0)return false;if(normaliserV377_(x.cible)==='charge_fixe')return true;const lib=normaliserV377_(x.libelle||''),cat=normaliserV377_(x.categorie||'');const mot=/(suspension|suspend|report|reporte|suppression|supprime|annulation|annule|echeance)/.test(lib);if(!mot)return false;const refs=tableauCerbereV379_(refLignes),masters=tableauCerbereV379_(charges);if(cat&&refs.some(c=>{const cc=normaliserV377_(c&&c.categorie||'');return cc===cat||memeFamilleCfV3717_(cat,cc);}))return true;return masters.some(c=>{const actif=!(c&&c.actif===false||normaliserV377_(c&&c.actif)==='false'||normaliserV377_(c&&c.actif)==='non');if(!actif)return false;const nom=normaliserV377_(c&&c.libelle_bancaire||c&&c.libelle||'');if(!nom||nom.length<4)return false;return lib.indexOf(nom)>=0||nom.indexOf(lib)>=0;});}

function construireLiensCfV3717_(operations,charges,rapprochements){const out=typeof construireLiensCfV3713_==='function'?construireLiensCfV3713_(operations,charges,rapprochements):construireLiensCfFortsV3711_(operations,charges,rapprochements);const cs=tableauCerbereV379_(charges).filter(c=>!(c&&c.actif===false||normaliserV377_(c&&c.actif)==='false'||normaliserV377_(c&&c.actif)==='non'));tableauCerbereV379_(operations).forEach(o=>{const id=String(o&&o.id||'').trim();if(!id||out[id]||Number(o&&o.montant||0)>=0)return;const cat=String(o&&o.categorie||'').trim(),mont=Math.abs(Number(o&&o.montant||0)),d=dateOperationBanqueV377_(o),nom=normaliserV377_(o&&o.libelle||o&&o.libelle_bancaire);if(!mont||!d)return;const parFamille=cs.filter(c=>memeFamilleCfV3717_(cat,c&&c.categorie)&&Math.abs(Math.abs(Number(c&&c.montant||0))-mont)<=.011&&distanceJourMoisV3714_(d.getDate(),jourCfV3714_(c)||d.getDate())<=7);if(parFamille.length===1){out[id]=String(parFamille[0].id||'');return;}if(nom&&nom.length>=4){const exactNom=cs.filter(c=>{const cn=normaliserV377_(c&&c.libelle_bancaire||c&&c.libelle);if(!cn||cn.length<4)return false;const procheNom=cn===nom||nom.indexOf(cn)>=0||cn.indexOf(nom)>=0;if(!procheNom)return false;const jour=jourCfV3714_(c);return !jour||distanceJourMoisV3714_(d.getDate(),jour)<=10;});if(exactNom.length===1)out[id]=String(exactNom[0].id||'');}});return out;}
...
function recalculerDerivesCerbereV3717_(p){if(!p||typeof p!=='object')return;const v=p.v37||(p.v37={}),env=Array.isArray(p.enveloppes)?p.enveloppes:[];v.dt1=arrV374_(Number(v.cft1||0)+Number(v.dpt1||0)+Number(v.het1||0));v.sct1=arrV374_(Number(v.ss1||0)+Number(v.rt1||0)-v.dt1);v.rpt1=v.sct1;v.resteReellementPilotable=v.sct1;v.disponibleJusquau27=v.sct1;v.capaciteAvantPilotable=arrV374_(Number(v.ss1||0)+Number(v.rt1||0)-Number(v.cft1||0)-Number(v.het1||0));const allocation=arrV374_(env.reduce((s,x)=>s+Math.max(0,Number(x&&x.prevu||0)),0)),dep=arrV374_(env.reduce((s,x)=>s+Math.max(0,Number(x&&x.engageV37||0)-Math.max(0,Number(x&&x.prevu||0))),0));v.allocationP1Courante=allocation;v.depassementsPilotables=dep;v.aReequilibrer=arrV374_(v.capaciteAvantPilotable-allocation-dep);v.aReequilibrerReference=v.aReequilibrer;const abs=arrV374_(env.reduce((s,x)=>s+Math.max(0,Number(x&&x.prevu||0)-Number(x&&x.engageV37||0)),0));v.absorbableParAllocations=abs;v.incompressible=arrV374_(v.aReequilibrer<0?Math.max(0,Math.abs(v.aReequilibrer)-abs):0);v.auditReequilibrage={attendu:v.sct1,reel:v.aReequilibrer,ecart:arrV374_(v.aReequilibrer-v.sct1),ok:Math.abs(v.aReequilibrer-v.sct1)<.011};p.resteBudgetPilotable=Number(v.ret1||0);p.resteReellementPilotable=v.sct1;p.capacitePilotable=v.sct1;p.capaciteTresorerie=v.sct1;}
