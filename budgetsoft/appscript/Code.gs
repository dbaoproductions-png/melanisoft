...
const TABLES = {
  ...
  Charges_fixes: ['id', 'libelle', 'categorie', 'compte', 'montant', 'type', 'jour_execution', 'date_debut', 'date_fin', 'actif', 'commentaire', 'frequence', 'libelle_bancaire', 'tolerance', 'nature'],
...
/**
 * Lecture pure : cette fonction NE DOIT JAMAIS écrire dans Operations.
 * Les Charges_fixes sont des données de prévision ; leur matérialisation en
 * opérations réelles ne peut être déclenchée que par une action explicite.
 */
function chargerToutesLesDonnees(){
  ...
  resultat.meta={
    version:BUDGETSOFT_VERSION,
    chargeLe:new Date().toISOString(),
    generation:{creees:0,ignorees:0,erreurs:[],mode:'lecture_seule'}
  };
  return resultat;
}
...
/**
 * Action explicite conservée pour compatibilité. Elle n'est plus appelée par
 * chargerToutesLesDonnees(). Toute invocation doit être volontaire.
 */
