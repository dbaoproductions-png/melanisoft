...
function construireCfCanoniquePourSnapshot20260914_(p,v,sources){
  ...
  (charges||[]).forEach(c=>{
    ...
    let occs=[];
    try{
      occs=(calculerEcheancesChargeFixeAjustees_(c,debut,fin,fin)||[]).filter(e=>{const d=dateCfSnapshotBuild20260914_(e&&e.date);return d&&jourCfSnapshotBuild20260914_(d)>=jourCfSnapshotBuild20260914_(debut)&&jourCfSnapshotBuild20260914_(d)<=jourCfSnapshotBuild20260914_(fin);});
...
function normaliserCerbereCfPourSnapshot20260914_(base,sources){
  ...
  if(p1&&d1){
    const a1=construireCfCanoniquePourSnapshot20260914_(p1,v1,sources);
  ...
  if(ps.length>1){
    ...
    if(p2&&d2){
      const a2=construireCfCanoniquePourSnapshot20260914_(p2,v2,sources);
  ...
  base.diagnostic.cfSnapshotBuild20260914={ok:true,version:BUDGETSOFT_CERBERE_CF_SNAPSHOT_BUILD_20260914_VERSION,doctrine:'Normalisation effectuée uniquement à la construction du snapshot ; lecture Cerbère inchangée et rapide.'};
  return base;
}
