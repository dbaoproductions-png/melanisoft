const BUDGETSOFT_REVENUE_PUBLICATION_FIX_20260912_VERSION='2026-09-12.3';
...
function evenementsRecettesCertainesDuesRevenuePublicationFix20260912_(reference,finCycle){
...
/**
 * Correctif terminal du propriétaire bancaire.
 * IMPORTANT : conserve les DEUX arguments du moteur canonique, notamment cerberePrecharge.
 */
construireTrajectoireTresorerieCanoniqueBudgetSoft20260907=function(dateCible,cerberePrecharge){
  const moteur=(typeof construireTrajectoireAvantRevenueFinal20260912_==='function')
    ?construireTrajectoireAvantRevenueFinal20260912_
    :null;
  if(!moteur)return{ok:false,erreur:'Propriétaire canonique historique indisponible.'};
  const r=moteur(dateCible,cerberePrecharge);if(!r||r.ok===false)return r;
  const reference=dateRevenuePublicationFix20260912_(r.dateReference||new Date()),cible=dateRevenuePublicationFix20260912_(dateCible||r.dateCible);if(!reference||!cible)return r;
  const finCycle=typeof dateFinCycleCanonBudgetSoft20260906_==='function'?dateFinCycleCanonBudgetSoft20260906_(reference):cible;
  const dus=evenementsRecettesCertainesDuesRevenuePublicationFix20260912_(reference,finCycle),lignes=Array.isArray(r.lignes)?r.lignes.slice():[];
  const report=new Date(reference);report.setDate(report.getDate()+1);report.setHours(12,0,0,0);
  dus.forEach(function(ev){
    const id=String(ev.id||''),montant=Math.abs(Number(ev.montant||0));if(!id||!montant)return;
    const deja=lignes.some(function(l){const d=dateRevenuePublicationFix20260912_(l&&l.date);return String(l&&l.source||'')==='evenement'&&String(l&&l.sourceId||'')===id&&d&&d>reference&&d<=cible&&Number(l&&l.montantSigne||0)>0;});
    if(deja)return;
  ...
  if(typeof decomposerTrajectoireTresorerieCanoniqueBudgetSoft20260907_==='function')r.decompositionCanonique=decomposerTrajectoireTresorerieCanoniqueBudgetSoft20260907_(r);
  return r;
};

/**
 * Garde de prépublication : la candidate n'a pas encore de revisionBudgetSoft.
 * La révision est contrôlée après publication par les audits intermodules ; ici on
 * bloque uniquement les divergences métier de la candidate en construction.
 */
verifierSupradoctrineRecettesBudgetSoft20260912_=function(etat){
  ...
  const reference=dateRevenuePublicationFix20260912_(ct.dateReference||new Date()),fin=dateRevenuePublicationFix20260912_(ct.fin||periode.fin),lignes=Array.isArray(proj.lignes)?proj.lignes:[],dus=evenementsRecettesCertainesDuesRevenuePublicationFix20260912_(reference,fin),controles=[];
...
function auditerCorrectifPublicationRecettesBudgetSoft20260912(){
  const s=chargerSnapshotGlobalBudgetSoft20260906();
  const out={ok:false,version:BUDGETSOFT_REVENUE_PUBLICATION_FIX_20260912_VERSION,revisionBudgetSoft:s&&s.revisionBudgetSoft||'',message:'Relancer actualiserBudgetSoftSyntheseMaintenant20260907 puis auditerRecettesIntermodulesBudgetSoft20260912.'};
  console.log('[AUDIT correctif publication recettes] '+JSON.stringify(out));return out;
}
