const CERBERE_EXPRESS_VIEW_VERSION = '2026-09-06.1';

/**
 * Vue interne Google Sheets : priorité à la dernière révision globale cohérente.
 * Pendant la reconstruction du snapshot global, le lecteur commun retourne null
 * et le calcul frais est exécuté afin de produire la nouvelle révision.
 */
function chargerVueCerbereExpress20260827(){
  ...
  if(global){global.performance=Object.assign({},global.performance||{},{source:'snapshot_global',dureeMs:0});return global;}
...
/** Calcul frais utilisé aussi par le rafraîchissement du snapshot public/global. */
...
function auditerVueCerbereExpress20260827(){const v=chargerVueCerbereExpress20260827();const out={ok:!!(v&&v.ok),version:v&&v.version,moteurVersion:v&&v.moteurVersion,sourceBudgetSoft:v&&v.sourceBudgetSoft||'',revisionBudgetSoft:v&&v.revisionBudgetSoft||'',cycle:v&&v.cycle,meteo:v&&v.meteo,consigneSaillante:v&&v.consigneSaillante,pilotable:v&&v.pilotable?{allocation:v.pilotable.allocation,consomme:v.pilotable.consomme,reste:v.pilotable.reste,aVentiler:v.pilotable.aVentiler}:null,pluxee:v&&v.pluxee,contexte:v&&v.contexte};console.log(JSON.stringify(out));return out;}
