...
function construireSyntheseComptes20260828_(){
  ...
  comptes.forEach(function(c){
    ...
    refs[id]={disponible:Number.isFinite(base)&&date&&!isNaN(date),solde:Number.isFinite(base)?base:null,date:date&&!isNaN(date)?date:null};
...
function auditerPerformanceComptesRapide20260828(){
  const t0=Date.now();const r=chargerSyntheseComptes20260828();
  const out={ok:r.ok===true,version:r.version,dureeMs:Date.now()-t0,source:r.performance&&r.performance.source||'',snapshotPerime:!!r.snapshotPerime,revisionBudgetSoft:r.revisionBudgetSoft||'',synthese:r.synthese,comptes:r.comptes.length,controleDashboardExecute:false};
  console.log(JSON.stringify(out));return out;
}

function auditerSyntheseComptes20260828(){
  const tComptes=Date.now();const r=construireSyntheseComptes20260828_();const dureeComptesMs=Date.now()-tComptes;
  let dashboardSolde=null,dashboardErreur='',dureeDashboardMs=null;const tDashboard=Date.now();
