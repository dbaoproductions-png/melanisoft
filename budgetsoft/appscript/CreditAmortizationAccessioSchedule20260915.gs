const ACCESSIO_SCHEDULE_20260915_VERSION='2026-09-16.1';
...
function ventilationEcheancierAccessio20260915_(credit,operation){
  ...
  // La banque comptabilise le prélèvement le 04/09 ; le libellé SEPA porte l'échéance contractuelle 05/09.
  if(cle!=='2026-09-04'&&cle!=='2026-09-05')return null;
...
function auditerMoteurAccessioExact20260915(){
  ...
  const tests=['2026-09-04','2026-09-05'].map(date=>({date,calcul:calculerAmortissementCredit20260915_(credit,{montant:-33.60,date_comptable:date})}));
  const ok=tests.every(t=>t.calcul&&t.calcul.methode&&String(t.calcul.methode).indexOf('releve_exact_ACCESSIO_')===0&&Number(t.calcul.partCapital)===23.47);
  const out={ok,version_moteur:typeof CREDIT_AMORTIZATION_INTERMODULE_20260915_VERSION!=='undefined'?CREDIT_AMORTIZATION_INTERMODULE_20260915_VERSION:'',version_releve:ACCESSIO_SCHEDULE_20260915_VERSION,credit:{id:credit.id,nom:credit.nom,capital_actuel:Number(credit.capital_restant||0)},tests,attendu:{montant:33.60,partCapital:23.47,partInterets:10.13,partAssurance:0,capitalAvant:800,capitalApres:776.53}};
