...
function cleCategorieBudgetSoft_(valeur){
  return String(valeur||'').trim().toLowerCase();
}

function categorieCibleBudgetSoft_(nom){
  const brut=String(nom||'').trim();
  return RENOMMAGES_CATEGORIES_BUDGETSOFT_[cleCategorieBudgetSoft_(brut)]||brut;
}

function typeCategorieCibleBudgetSoft_(nom,typeActuel){
  const cible=CATEGORIES_BUDGETSOFT_CIBLES_.find(c=>cleCategorieBudgetSoft_(c.nom)===cleCategorieBudgetSoft_(nom));
  return cible?cible.type:String(typeActuel||'').trim();
}

function migrerReferencesCategoriesBudgetSoft_(renommages){
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  const feuilles=['Operations','Charges_fixes','Correspondances_bancaires','Regles_categories'];
  let cellulesModifiees=0;
  feuilles.forEach(nomFeuille=>{
    const f=ss.getSheetByName(nomFeuille);
    if(!f||f.getLastRow()<2||f.getLastColumn()<1)return;
    const entetes=f.getRange(1,1,1,f.getLastColumn()).getValues()[0].map(v=>String(v||'').trim());
    const index=entetes.indexOf('categorie');
    if(index<0)return;
    const plage=f.getRange(2,index+1,f.getLastRow()-1,1);
    const valeurs=plage.getValues();
    let change=false;
    valeurs.forEach(r=>{
      const avant=String(r[0]||'').trim();
      if(!avant)return;
      const apres=renommages[cleCategorieBudgetSoft_(avant)]||avant;
      if(apres!==avant){r[0]=apres;cellulesModifiees++;change=true;}
    });
...
function installerArchitectureCategoriesBudgetSoft(){
  ...
  const renommees=[];
  lignes.forEach(r=>{
    const ancienNom=String(r[1]||'').trim();
    if(!ancienNom)return;
    const nouveauNom=categorieCibleBudgetSoft_(ancienNom);
    if(nouveauNom!==ancienNom)renommees.push({de:ancienNom,vers:nouveauNom});
    const type=typeCategorieCibleBudgetSoft_(nouveauNom,r[2]);
    const cle=cleCategorieBudgetSoft_(nouveauNom);
    const existante=parNom.get(cle);
    const objet={id:r[0]||Utilities.getUuid(),nom:nouveauNom,type,couleur:r[3]||'',actif:r[4]!==false&&String(r[4]).toLowerCase()!=='false'};
    if(!existante){parNom.set(cle,objet);return;}
    if(!existante.couleur&&objet.couleur)existante.couleur=objet.couleur;
    existante.actif=existante.actif||objet.actif;
    if(!existante.type&&objet.type)existante.type=objet.type;
  });

  const ajoutees=[];
  CATEGORIES_BUDGETSOFT_CIBLES_.forEach(c=>{
    const cle=cleCategorieBudgetSoft_(c.nom);
    if(parNom.has(cle)){
      parNom.get(cle).type=c.type;
      return;
    }
    parNom.set(cle,{id:Utilities.getUuid(),nom:c.nom,type:c.type,couleur:'',actif:true});
    ajoutees.push(c.nom);
  });

  const ordre=new Map(CATEGORIES_BUDGETSOFT_CIBLES_.map((c,i)=>[cleCategorieBudgetSoft_(c.nom),i]));
  const finales=[...parNom.values()].sort((a,b)=>{
    const ia=ordre.has(cleCategorieBudgetSoft_(a.nom))?ordre.get(cleCategorieBudgetSoft_(a.nom)):999;
    const ib=ordre.has(cleCategorieBudgetSoft_(b.nom))?ordre.get(cleCategorieBudgetSoft_(b.nom)):999;
    return ia-ib||a.nom.localeCompare(b.nom,'fr');
  });

  if(feuille.getLastRow()>1)feuille.getRange(2,1,feuille.getLastRow()-1,Math.max(feuille.getLastColumn(),entetes.length)).clearContent();
  if(finales.length)feuille.getRange(2,1,finales.length,entetes.length).setValues(finales.map(c=>[c.id,c.nom,c.type,c.couleur,c.actif]));
  feuille.setFrozenRows(1);
  feuille.autoResizeColumns(1,entetes.length);

  return{
    ok:true,
    categories:finales.length,
    ajoutees,
    renommees,
    referencesModifiees,
