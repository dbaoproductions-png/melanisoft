const FIXED_CHARGES_REVIEW_20260828_VERSION='2026-08-28.1';
...
function chargerChargesFixesReview20260828(){
  ...
  const cycleCourant=cycle28DepuisDate20260828_(derniereDate);
  const cyclePrecedent=cyclePrecedent20260828_(cycleCourant);
  const reelParCharge={};
  let reelCourant=0,reelPrecedent=0;
  ...
  const lignes=charges.map(c=>{
    const r=reelParCharge[String(c.id)]||{};
    const precedent=Number(r.precedent||0),courant=Number(r.courant||0),reference=Math.abs(Number(c.montant||0));
    const tolerance=Math.max(Number(c.tolerance||.5),Math.max(1,reference*.05));
    return Object.assign({},c,{
      reel_cycle_precedent:precedent?Math.round(precedent*100)/100:null,
      reel_cycle_courant:courant?Math.round(courant*100)/100:null,
      ecart_precedent:precedent?Math.round((precedent-reference)*100)/100:null,
      alerte_precedent:!!precedent&&Math.abs(precedent-reference)>tolerance
    });
  });

  return {
    ok:true,version:FIXED_CHARGES_REVIEW_20260828_VERSION,
    Charges_fixes:lignes,Comptes:comptes,Categories:categories,
    synthese:{
      chargesActives:actives.length,
      montantMensuel:Math.round(actives.reduce((s,c)=>s+equivalentMensuelChargeFixe20260828_(c),0)*100)/100,
      reelCyclePrecedent:Math.round(reelPrecedent*100)/100,
      reelCycleCourant:Math.round(reelCourant*100)/100
    },
    cycles:{
      courant:{debut:formatDateLocaleBudgetSoft_(cycleCourant.debut),fin:formatDateLocaleBudgetSoft_(cycleCourant.fin)},
      precedent:{debut:formatDateLocaleBudgetSoft_(cyclePrecedent.debut),fin:formatDateLocaleBudgetSoft_(cyclePrecedent.fin)},
      derniereDateBanque:formatDateLocaleBudgetSoft_(derniereDate)
    },
    _performance:{serveurMs:Date.now()-t0,charges:charges.length,operations:operations.length}
  };
}
...
function sauvegarderChargeFixeReview20260828(charge){
  ...
  ['dernier_rapprochement_id','dernier_rapprochement_date','dernier_montant_reel','statut_rapprochement'].forEach(k=>{
    if(existante&&charge[k]===undefined)fusion[k]=existante[k];
  });
...
function chargerPropositionsRapprochementChargesFixes20260828(){
  ...
  const courant=cycle28DepuisDate20260828_(derniere),precedent=cyclePrecedent20260828_(courant);
  const debutRecherche=precedent.debut;
  const historique=typeof lireRapprochementsChargesFixes==='function'?lireRapprochementsChargesFixes():[];
  const traites=new Set(historique.filter(r=>String(r.statut)!=='À valider').map(r=>String(r.charge_fixe_id)+'|'+String(r.operation_id)));
  const candidats=[];
  const opsCandidates=operations.filter(o=>{
    if(chargeFixeLieeOperation20260828_(o))return false;
    const d=dateComptableOperation20260828_(o);return d&&d>=debutRecherche&&d<=courant.fin;
  });

  charges.forEach(charge=>{
    opsCandidates.forEach(operation=>{
      const cle=String(charge.id)+'|'+String(operation.id);if(traites.has(cle))return;
      const clone=Object.assign({},operation,{date:operation.date_comptable||operation.date});
      const r=evaluerRapprochementChargeFixe_(charge,clone);if(!r||r.score<55)return;
      candidats.push(Object.assign({},r,{charge_fixe_id:String(charge.id),operation_id:String(operation.id)}));
    });
  });
  const meilleure=new Map();
  candidats.sort((a,b)=>Number(b.score)-Number(a.score)).forEach(c=>{if(!meilleure.has(c.operation_id))meilleure.set(c.operation_id,c)});
  const propositions=[...meilleure.values()];
  return {ok:true,propositions,nombre:propositions.length,_performance:{serveurMs:Date.now()-t0,operationsCandidates:opsCandidates.length,comparaisons:charges.length*opsCandidates.length}};
}
...
function deciderRapprochementChargeFixeRapide20260828(chargeId,operationId,decision){
  ...
  const charges=lireTable_('Charges_fixes'),charge=charges.find(c=>String(c.id)===String(chargeId));
  if(!charge)throw new Error('Charge fixe introuvable.');

  const fOps=SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Operations');
  const hOps=TABLES.Operations,idxId=hOps.indexOf('id'),ids=fOps.getRange(2,idxId+1,Math.max(0,fOps.getLastRow()-1),1).getValues().flat();
  const p=ids.findIndex(v=>String(v)===String(operationId));if(p<0)throw new Error('Opération introuvable.');
  const ligne=p+2,valeurs=fOps.getRange(ligne,1,1,hOps.length).getValues()[0];
  const operation=Object.fromEntries(hOps.map((h,i)=>[h,serialiserValeur_(valeurs[i])]));

  if(choix==='valider'){
    const colLien=hOps.indexOf('charge_fixe_id')+1,colStatut=hOps.indexOf('statut_bancaire')+1;
    if(colLien>0)fOps.getRange(ligne,colLien).setValue(String(chargeId));
    if(colStatut>0&&String(operation.source_bancaire||'').toLowerCase()!=='manuel')fOps.getRange(ligne,colStatut).setValue('rapprochee_charge_fixe');
    ...
    if(pc>=0){
      const no=pc+2,d=dateComptableOperation20260828_(operation),m=Math.abs(Number(operation.montant||0));
      [['dernier_rapprochement_id',operationId],['dernier_rapprochement_date',d?formatDateLocaleBudgetSoft_(d):''],['dernier_montant_reel',m],['statut_rapprochement','Rapprochée']].forEach(([k,v])=>{const c=hCf.indexOf(k)+1;if(c>0)fCf.getRange(no,c).setValue(v)});
    }
  }

  if(typeof initialiserRapprochementsChargesFixes_==='function'){
    const f=initialiserRapprochementsChargesFixes_(),h=FIXED_CHARGE_MATCH_HEADERS;
    const d=dateComptableOperation20260828_(operation),m=Math.abs(Number(operation.montant||0)),att=Math.abs(Number(charge.montant||0));
    const row={id:Utilities.getUuid(),charge_fixe_id:chargeId,operation_id:operationId,score:'',statut:choix==='valider'?'Validé':'Ignoré',date_operation:d?d.toISOString():'',montant_reel:m,montant_attendu:att,ecart_montant:Math.round(Math.abs(m-att)*100)/100,ecart_jours:'',libelle_operation:String(operation.libelle||''),libelle_charge:String(charge.libelle||''),compte:String(operation.compte||''),decision:choix==='valider'?'Rapproché à l’opération réelle':'Proposition ignorée',cree_le:new Date().toISOString(),modifie_le:new Date().toISOString()};
    f.getRange(f.getLastRow()+1,1,1,h.length).setValues([h.map(k=>row[k]??'')]);
  }
