...
function calculerCycleDepuisSalaire_(reference,operations,jourRepli,salairePrincipal){const dateRef=reference instanceof Date?reference:new Date(reference||new Date());const filtre=salairePrincipal?o=>estSalairePrincipal_(o,salairePrincipal):o=>estOperationSalaire_(o);const salaires=(operations||[]).filter(filtre).map(o=>new Date(o.date)).filter(d=>!isNaN(d)).sort((a,b)=>a-b);const precedent=salaires.filter(d=>d<=dateRef).pop(),suivant=salaires.find(d=>d>dateRef);if(precedent){let debut=new Date(precedent),fin;if(suivant)fin=new Date(suivant.getTime()-1);else{const jourEstime=Math.max(1,Math.min(28,Number(salairePrincipal&&salairePrincipal.jourMoyen)||Number(jourRepli)||28));let prochainEstime=new Date(debut.getFullYear(),debut.getMonth()+1,jourEstime,0,0,0,0);if(prochainEstime<=debut)prochainEstime=new Date(debut.getFullYear(),debut.getMonth()+2,jourEstime,0,0,0,0);fin=new Date(prochainEstime.getTime()-1);while(fin<dateRef){debut=new Date(prochainEstime);prochainEstime=new Date(debut.getFullYear(),debut.getMonth()+1,jourEstime,0,0,0,0);fin=new Date(prochainEstime.getTime()-1);}}return construirePeriodeCycle_(debut,fin,true);}const jour=Math.max(1,Math.min(28,Number(jourRepli)||28));const debut=dateRef.getDate()>=jour?new Date(dateRef.getFullYear(),dateRef.getMonth(),jour):new Date(dateRef.getFullYear(),dateRef.getMonth()-1,jour);const fin=new Date(new Date(debut.getFullYear(),debut.getMonth()+1,jour).getTime()-1);return construirePeriodeCycle_(debut,fin,false);}
