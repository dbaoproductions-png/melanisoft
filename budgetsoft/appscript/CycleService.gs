...
function detecterSalairePrincipal_(operations, moisHistorique) {
  ...
  const candidats=(operations||[]).filter(estOperationSalaire_).map(o=>({operation:o,date:new Date(o.date_comptable||o.date),montant:Math.abs(Number(o.montant||0)),cle:cleSalaire_(o)})).filter(c=>!isNaN(c.date)&&c.date>=limite);
...
function calculerCycleDepuisSalaire_(reference,operations,jourRepli,salairePrincipal){
  // Convention globale BudgetSoft étalonnée sur le salaire principal : le jour le
  // plus tardif observé est le 28. La date réelle d'un versement donné (23, 26, 27…)
  // ne décale donc plus le mois correspondant. Tous les modules utilisent 28 → 27.
  const dateRef=reference instanceof Date&&!isNaN(reference)?reference:new Date(reference||new Date());
  const jour=typeof jourDebutCycleBudgetSoft_==='function'?jourDebutCycleBudgetSoft_():28;
  let debut=dateRef.getDate()>=jour?new Date(dateRef.getFullYear(),dateRef.getMonth(),jour,0,0,0,0):new Date(dateRef.getFullYear(),dateRef.getMonth()-1,jour,0,0,0,0);
  const prochain=new Date(debut.getFullYear(),debut.getMonth()+1,jour,0,0,0,0);
  const fin=new Date(prochain.getTime()-1);
  return construirePeriodeCycle_(debut,fin,!!salairePrincipal);
}

function construirePeriodeCycle_(debut,fin,salaireDetecte){const maintenant=new Date(),debutJour=dateJourCycle_(debut),finJour=dateJourCycle_(fin),maintenantJour=dateJourCycle_(maintenant),duree=Math.max(1,ecartJoursCycle_(debutJour,finJour)+1),jourCourant=maintenantJour<debutJour?0:maintenantJour>finJour?duree:Math.min(duree,ecartJoursCycle_(debutJour,maintenantJour)+1),moisNom=new Date(debut.getFullYear(),debut.getMonth()+1,1),libelle=moisNom.toLocaleDateString('fr-FR',{month:'long',year:'numeric'}).replace(/^./,c=>c.toUpperCase());return {debut:debut.toISOString(),fin:fin.toISOString(),cle:Utilities.formatDate(moisNom,Session.getScriptTimeZone(),'yyyy-MM'),libelle,salaireDetecte,jourReference:typeof jourDebutCycleBudgetSoft_==='function'?jourDebutCycleBudgetSoft_():28,conventionCycle:'28 inclus → 27 inclus',dureeJours:duree,jourCourant,termine:maintenantJour>finJour,progression:Math.max(0,Math.min(100,Math.round(jourCourant/duree*100))),joursRestants:Math.max(0,ecartJoursCycle_(maintenantJour,finJour))};}
