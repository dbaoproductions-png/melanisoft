...
function estOperationCarte_(operation) {
  ...
  // Une mention commerciale comme « Carrefour Banque » ne suffit jamais.
  // Seules les vraies lignes de facture carte Hello bank sont déplacées au dernier jour du mois.
...
function construirePeriodeCycle_(debut,fin,salaireDetecte){const maintenant=new Date(),duree=Math.max(1,Math.ceil((fin-debut)/86400000)+1),jourCourant=maintenant<debut?0:maintenant>fin?duree:Math.min(duree,Math.floor((maintenant-debut)/86400000)+1),moisNom=new Date(debut.getFullYear(),debut.getMonth()+1,1),libelle=moisNom.toLocaleDateString('fr-FR',{month:'long',year:'numeric'}).replace(/^./,c=>c.toUpperCase());return {debut:debut.toISOString(),fin:fin.toISOString(),cle:Utilities.formatDate(moisNom,Session.getScriptTimeZone(),'yyyy-MM'),libelle,salaireDetecte,dureeJours:duree,jourCourant,progression:Math.max(0,Math.min(100,Math.round(jourCourant/duree*100))),joursRestants:Math.max(0,Math.ceil((fin-maintenant)/86400000))};}
