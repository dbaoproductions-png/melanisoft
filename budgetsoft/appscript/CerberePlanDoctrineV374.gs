...
function previsionsEvenementsV371_(events,periode,p0Cats,catType){
  ...
  function ajouterOccurrence_(x,o,source){let impact=o.date;const typeExplicite=normaliserV37_(x.type||x.sens||''),impactType=normaliserV37_(x.impact_type||''),nature=normaliserV37_(x.nature_action||''),fonction=normaliserV37_(x.fonction_plan||''),actionRevenu=source==='Action'&&(impactType==='hausse revenu'||impactType==='hausse_revenu'||nature==='recevoir'||nature==='encaisser'||nature==='recouvrer'||fonction==='recevoir'),estRecette=source==='Événement'&&typeExplicite==='recette',estDepense=typeExplicite==='depense'||['acheter','rembourser','reserver','investir','payer'].includes(nature);if(actionRevenu)return;if(source==='Action'&&!estRecette&&!estDepense)return;if(!estRecette&&normaliserV37_(x.mode_paiement)==='cb')impact=dateImpactCbPlanV37_(o.date);if(!dateDansPeriodeV37_(impact,periode))return;const dateImpact=dateValideVentilationBudgetSoft_(impact),futur=!dateImpact||dateImpact>maintenant,m=Math.abs(Number(o.montant||0)),cat=String(x.categorie||'').trim();if(!m)return;if(estRecette){out.recettesInitiales+=m;if(futur)out.recettesFutures+=m;if(source==='Action')out.recettesActions+=m;if(/sante|remboursement/.test(normaliserV37_(cat+' '+(x.libelle||''))))out.remboursementsSante+=m;return;}if(p0Cats.has(cat)){out.depensesPilotablesParCategorie[cat]=(out.depensesPilotablesParCategorie[cat]||0)+m;if(futur)out.depensesFutures+=m;}else{out.depensesHorsPilotable+=m;if(futur)out.depensesHorsPilotableFutures+=m;}if(source==='Action')out.depensesActions+=m;}
  (events||[]).forEach(e=>{if(estNeutralise_(e)&&!(yesPlanV4_(e.fractionne)&&typeof occurrencesEvenementEtatBudgetSoft20260922_==='function')){out.previsionsNeutraliseesParReel++;return;}if(String(e.certitude||'certaine')==='possible')return;out.evenementsRetenus++;const occ=typeof occurrencesEvenementEtatBudgetSoft20260922_==='function'?occurrencesEvenementEtatBudgetSoft20260922_(e):(typeof occurrencesEvenementV4_==='function'?occurrencesEvenementV4_(e):[{index:1,montant:Math.abs(Number(e.montant||0)),date:e.date_effet}]);occ.forEach(o=>{if(o&&o.rapprochee){out.previsionsNeutraliseesParReel++;return;}ajouterOccurrence_(e,o,'Événement');});});
