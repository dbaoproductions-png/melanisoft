...
function auditerEvenementTennisReel20260922(){
  ...
  const controles=[
    ...
    {code:'PROJECTION_TRESORERIE_RESPECTE_HORIZON',ok:(function(){const cible=new Date(proj&&proj.dateCible||0);if(isNaN(cible))return false;const attendues=occ.filter(function(o){const d=new Date(o&&o.date||0);return !isNaN(d)&&d<=cible&&!o.rapprochee;});const totalAttendu=arr(attendues.reduce(function(s,o){return s- Math.abs(Number(o&&o.montant||0));},0));const totalProj=arr(projTennis.reduce(function(s,x){return s+Number(x&&x.montantSigne||0);},0));return Math.abs(totalProj-totalAttendu)<=.011&&projTennis.length===attendues.length;})(),detail:{dateCible:proj&&proj.dateCible||'',projection:projTennis.map(function(x){return{occurrence:x.occurrence,date:x.date,montantSigne:x.montantSigne,enRetard:!!x.enRetard};}),occurrencesTotales:occ.map(function(o){return{occurrence:o.index,date:o.date,montant:o.montant,rapprochee:!!o.rapprochee};})}},
