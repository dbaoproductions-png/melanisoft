...
function construireCreditsEtDettesV2_() {
  ...
  const somme=(xs,champ)=>Math.round((xs||[]).reduce((s,x)=>s+Math.max(0,Number(x&&x[champ]||0)),0)*100)/100;
  const capitalAmortissable=somme(amortissables,'capital_restant'),mensualitesAmortissables=somme(amortissables,'mensualite'),coutAmortissable=somme(amortissables,'cout_restant'),echeancesAmortissables=Math.round(amortissables.reduce((s,c)=>s+Math.max(0,Number(c.echeances_restantes||0)),0));
  const capitalRenouvelable=somme(renouvelables,'capital_restant'),mensualitesRenouvelables=somme(renouvelables,'mensualite'),coutRenouvelable=somme(renouvelables,'cout_restant'),plafondRenouvelable=somme(renouvelables,'plafond_credit'),disponibleRenouvelable=somme(renouvelables,'disponible_credit'),assuranceRenouvelable=somme(renouvelables,'assurance_mensuelle'),echeancesRenouvelables=Math.round(renouvelables.reduce((s,c)=>s+Math.max(0,Number(c.echeances_restantes||0)),0)),tauxRenouvelablePondere=capitalRenouvelable?renouvelables.reduce((s,c)=>s+Number(c.capital_restant||0)*Number(c.taux||0),0)/capitalRenouvelable:0;
  const alertes=analyserCoherenceCreditsV2_(credits,dettes);
  return {version:CREDITS_DATA_V2_VERSION,lignes:tous,capitalRestant,capitalCredits,dettesHorsCredit,endettementTotal:capitalRestant,mensualites,mensualitesCredits,mensualitesDettes,tauxPondere,echeancesRestantes,coutRestant,amortissables,renouvelables,dettes,dettesActives,capitalAmortissable,mensualitesAmortissables,coutAmortissable,echeancesAmortissables,capitalRenouvelable,mensualitesRenouvelables,coutRenouvelable,plafondRenouvelable,disponibleRenouvelable,assuranceRenouvelable,echeancesRenouvelables,tauxRenouvelablePondere,alertes,sourceBudgetSoft:'constructeur_snapshot'};
}
