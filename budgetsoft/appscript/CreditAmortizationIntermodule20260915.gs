const CREDIT_AMORTIZATION_INTERMODULE_20260915_VERSION='2026-09-15.9';
const CREDIT_AMORTIZATION_SHEET_20260915='Amortissements_credits';
const CREDIT_AMORTIZATION_HEADERS_20260915=['id','credit_id','credit_nom','charge_fixe_id','operation_id','date_operation','montant_echeance','part_capital','part_interets','part_assurance','capital_avant','capital_apres','echeances_avant','echeances_apres','prochaine_echeance_avant','prochaine_echeance_apres','disponible_avant','disponible_apres','plafond_credit','methode','statut','version','date_traitement'];
...
function calculerAmortissementCredit20260915_(credit,operation){
  ...
  const exactCarrefour=typeof ventilationReleveCarrefourPass20260915_==='function'?ventilationReleveCarrefourPass20260915_(credit,operation):null;if(exactCarrefour){const partCapital=Math.min(capital,Math.max(0,exactCarrefour.partCapital));return{montant:arrCreditAmort20260915_(montant),partCapital:arrCreditAmort20260915_(partCapital),partInterets:arrCreditAmort20260915_(exactCarrefour.partInterets),partAssurance:arrCreditAmort20260915_(exactCarrefour.partAssurance),capitalAvant:arrCreditAmort20260915_(capital),capitalApres:arrCreditAmort20260915_(capital-partCapital),capitalApresTheorique:arrCreditAmort20260915_(exactCarrefour.capitalApresTheorique),methode:exactCarrefour.methode};}
...
function appliquerAmortissementCreditDepuisRapprochement20260915_(chargeId,operationId){
  ...
  try{
    ...
    const disponibleAvant=Math.max(0,Number(credit.disponible_credit||0)),plafond=Math.max(0,Number(credit.plafond_credit||0));
    const disponibleApres=type==='revolving'
      ?arrCreditAmort20260915_(plafond>0?Math.min(plafond,disponibleAvant+Math.max(0,Number(calc.partCapital||0))):disponibleAvant+Math.max(0,Number(calc.partCapital||0)))
      :disponibleAvant;
    const cible=Object.assign({},credit,{capital_restant:calc.capitalApres,echeances_restantes:apresEch,prochaine_echeance:next||credit.prochaine_echeance});
    if(type==='revolving')cible.disponible_credit=disponibleApres;
    if(typeof enregistrerCreditEtendu_==='function')enregistrerCreditEtendu_(cible);else enregistrerLigne('Credits',cible);
    const sh=assurerJournalAmortissementsCredits20260915_(),row={id:Utilities.getUuid(),credit_id:credit.id,credit_nom:credit.nom,charge_fixe_id:cid,operation_id:oid,date_operation:dateOp,montant_echeance:calc.montant,part_capital:calc.partCapital,part_interets:calc.partInterets,part_assurance:calc.partAssurance,capital_avant:calc.capitalAvant,capital_apres:calc.capitalApres,echeances_avant:avantEch,echeances_apres:apresEch,prochaine_echeance_avant:credit.prochaine_echeance||'',prochaine_echeance_apres:next||'',disponible_avant:type==='revolving'?disponibleAvant:'',disponible_apres:type==='revolving'?disponibleApres:'',plafond_credit:type==='revolving'?plafond:'',methode:calc.methode,statut:'applique',version:CREDIT_AMORTIZATION_INTERMODULE_20260915_VERSION,date_traitement:new Date().toISOString()};
