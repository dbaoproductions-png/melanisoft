const CREDIT_AMORTIZATION_CATCHUP_20260915_VERSION='2026-09-15.2';
...
function simulerRattrapageAmortissementsCredits20260915(){
  ...
  credits.forEach(credit=>{
    ...
    const applicationAutorisee=confiance==='haute'&&exclues.length===0;
    lignes.push({credit_id:String(credit.id||''),credit:String(credit.nom||''),capital_actuel:capitalActuel,date_reference:refDate?Utilities.formatDate(refDate,Session.getScriptTimeZone(),'yyyy-MM-dd'):'',capital_reference:refCapital,confiance,raison,operations_a_rattraper:operations.length,operations_exclues:exclues.length,capital_a_deduire:Math.round(operations.reduce((s,x)=>s+Number(x.part_capital||0),0)*100)/100,capital_simule_apres:Math.round(Number(virtuel.capital_restant||0)*100)/100,application_automatique_autorisable:applicationAutorisee,operations,exclues});
  });
  const autorisables=lignes.filter(x=>x.application_automatique_autorisable),out={ok:true,version:CREDIT_AMORTIZATION_CATCHUP_20260915_VERSION,lecture_seule:true,nombre_credits:lignes.length,nombre_operations:lignes.reduce((s,x)=>s+x.operations_a_rattraper,0),nombre_operations_exclues:lignes.reduce((s,x)=>s+x.operations_exclues,0),capital_total_a_deduire:Math.round(lignes.reduce((s,x)=>s+x.capital_a_deduire,0)*100)/100,capital_total_autorisable:Math.round(autorisables.reduce((s,x)=>s+x.capital_a_deduire,0)*100)/100,credits:lignes};
