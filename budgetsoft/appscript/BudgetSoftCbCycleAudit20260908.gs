...
// Garde permanente anti-régression multi-cycle.
// Placée volontairement dans ce fichier déjà déployé par Apps Script : le mécanisme
// de déploiement TEST ne crée pas de manière fiable les nouveaux fichiers .gs.
...
function auditerGardeCbMultiCycleBudgetSoft20260908(dateFinCycleSuivant){
  ...
  const parDate={};
  publiees.forEach(x=>{const j=isoJourGardeCbMultiCycleBudgetSoft20260908_(x.date);(parDate[j]=parDate[j]||[]).push(x);});
  ...
  datesAttendues.forEach(j=>{const ls=parDate[j]||[];if(ls.length===0)erreurs.push({code:'CB_DEBIT_CYCLE_ABSENT',message:'Un débit CB attendu dans l’horizon n’est pas publié.',date:j});});
