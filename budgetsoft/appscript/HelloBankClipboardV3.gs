const HELLOBANK_CLIPBOARD_V3='3.0';
...
function hb3Ignorer_(s){return !s||/^cat[ée]gorie\s*libell[ée]\s*montant$/i.test(s)||/^(lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)\b/i.test(s);}
function hb3Parser_(texte,compte){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(Number.isFinite(amount)){
      ...
      if(j>=0)lib=lines[j];
      const dc=hb3Date_(debitLine),da=hb3Achat_(lib)||dc;
      if(!dc||!lib)continue;
      ...
      out.push({compte:String(compte||''),date:da,date_achat:da,date_comptable:dc,libelle_bancaire:lib,libelle:titreLibelle(marchand||lib),marchand_normalise:marchand,carte_fin:carte,montant:signed,type:signed<0?'depense':'revenu',source_bancaire:'flux',statut_bancaire:'provisoire'});
      lib='';continue;
    }
    lib=s;
  }
...
function importerCollerHelloBankV3(texte,compte){
  ...
  const res=upsertOperationsBancairesTransactionnel(nouvelles,compte,'flux',null);return Object.assign({ok:true,existantes:sim.existantes},res);
}
