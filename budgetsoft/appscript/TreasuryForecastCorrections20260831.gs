...
function prioriteSourceTresorerie20260831_(s){
  return {operation_future:0,evenement:1,action:2,revenu_canonique:3,revenu_recurrent:4,charge_fixe:5,pilotable:8}[String(s||'')]??9;
}
...
function ressemblentTresorerie20260831_(a,b){
  if(!a||!b)return false;
  // Deux références de même nature et d'identifiants différents sont deux flux distincts.
  // Ex. les 2 Google One à 1,99 € et les 3 assurances Avanssur.
...
function dedoublonnerPrevisionsTresorerie20260831_(lignes){
  ...
  const gardees=[];
  trie.forEach(x=>{if(x.source==='pilotable'){gardees.push(x);return;}if(gardees.some(y=>y.source!=='pilotable'&&ressemblentTresorerie20260831_(x,y)))return;gardees.push(x);});
  return gardees;
}
...
function appliquerSuppressionsTemporairesTresorerie20260831_(lignes,evenements){
  ...
  (evenements||[]).filter(e=>statutEffectifTresorerie20260831_(e.statut)&&estSuspensionTemporaireTresorerie20260831_(e)).forEach(e=>{
    ...
    // La suspension n'est jamais une recette : on retire sa ligne événementuelle.
    out=out.filter(x=>!(x.source==='evenement'&&String(x.sourceId||'')===eid));
    ...
    out.forEach((x,i)=>{
      ...
      const memeCat=cat&&String(x.categorie||'').trim()===cat;
      const le=normaliserLibelleTresorerie20260831_(e.libelle),lx=normaliserLibelleTresorerie20260831_(x.libelle),commun=le.split(' ').filter(w=>w.length>=4).some(w=>lx.includes(w));
...
function revenusCanoniquesTresorerie20260831_(ops,lignesExistantes,now,cible){
  let canon=[];try{canon=lireTable_('Cerbere_Recettes_Canon_V1');}catch(e){canon=[];}
  const out=[];
  (canon||[]).forEach(c=>{
    ...
    const hist=(ops||[]).map(o=>({o,d:dateOpTresorerie_(o),m:Math.abs(Number(o.montant||0))})).filter(x=>x.d&&x.d<=now&&x.d>=new Date(now.getFullYear(),now.getMonth()-6,1)&&(String(x.o.type||'').toLowerCase()==='revenu'||Number(x.o.montant||0)>0)&&String(x.o.categorie||'').trim()===cat);
    const baseMont=Math.abs(Number(c.montant||0));
