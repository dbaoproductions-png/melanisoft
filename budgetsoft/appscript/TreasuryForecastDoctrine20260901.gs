const TREASURY_FORECAST_DOCTRINE_20260901_VERSION='2026-09-13.1';
...
/**
 * Propriétaire du prévisionnel bancaire. La part pilotable non encore engagée
 * provient exclusivement d'EP. Elle est ventilée une seule fois : immédiat avant
 * frontière + CB différée à la vraie date de débit estimée.
 */
function chargerTresoreriePrevisionnelle20260901(dateCible,cerberePrecharge){
  ...
  lignes=lignes.filter(x=>!['debit_cb_estime','ep_immediat_estime'].includes(String(x&&x.source||'')));
  const projectionEp=projectionEnvelopePilotableTresorerie20260913_(ops,reference,cible,cerberePrecharge);
  if(projectionEp.immediat)lignes.push(projectionEp.immediat);
  if(projectionEp.debitsCb.length)lignes.push.apply(lignes,projectionEp.debitsCb);
...
function cerbereProjectionEpTresorerie20260913_(cerberePrecharge){let c=cerberePrecharge||null;if(!c){try{const chargeur=typeof chargerCerbereCockpit20260902==='function'?chargerCerbereCockpit20260902:(typeof chargerCerbereV374==='function'?chargerCerbereV374:null);if(chargeur)c=chargeur();}catch(e){c=null;}}if(c&&typeof enrichirEnvelopePilotableBudgetSoft20260913_==='function')try{enrichirEnvelopePilotableBudgetSoft20260913_(c);}catch(e){}return c;}

/** Construit la seule projection de l'EP restant ; Réel déjà engagé exclu. */
function projectionEnvelopePilotableTresorerie20260913_(ops,reference,cible,cerberePrecharge){
  ...
  if(!p||typeof calculerImpactPrevisionnelEpBudgetSoft20260913_!=='function')return{immediat:null,debitsCb:estimationsDebitsCbDiffereTresorerie20260908_(ops,reference,cible,c),diagnostic:{ownerEp:false,legacy:true}};
  ...
  return{immediat:immediat,debitsCb:debitsCb,diagnostic:{version:'2026-09-13.1',ownerEp:true,proprietaire:BUDGETSOFT_EP_OWNER_20260913,ep:Number(impact.ep||0),consomme:Number(impact.consomme||0),resteAEngager:Number(impact.resteAEngager||0),immediat:Number(impact.immediat||0),differe:Number(impact.differe||0),tauxDifferePct:Number(impact.tauxDifferePct||0),reconciliation:Number(impact.reconciliation||0),profil:impact.profil||null}};
}

/** Compatibilité : appels historiques reçoivent désormais le calcul EP canonique. */
