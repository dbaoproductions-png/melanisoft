...
function envoyerSmsOvhCerbereExpressAuxDeux20260827(){
  ...
  return {ok:true,version:CERBERE_EXPRESS_SMS_OVH_VERSION,principal:envoyerSmsOvhCerbereExpress20260827('principal'),conjointe:envoyerSmsOvhCerbereExpress20260827('conjointe')};
}
...
function requeteOvhSmsCerbereExpress20260827_(methode,path,body){
  ...
  const secret=String(p.getProperty(CERBERE_EXPRESS_SMS_OVH_PROPS.applicationSecret)||'');
  const ck=String(p.getProperty(CERBERE_EXPRESS_SMS_OVH_PROPS.consumerKey)||'');
  if(!ak||!secret||!ck)throw new Error('Identifiants API OVH SMS incomplets dans les propriétés du script.');
  ...
  const aSigner=[secret,ck,methode,url,body,String(timestamp)].join('+');
  const digest=Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_1,aSigner,Utilities.Charset.UTF_8);
  const signature='$1$'+digest.map(function(b){return('0'+((b+256)%256).toString(16)).slice(-2);}).join('');
  const options={method:methode.toLowerCase(),muteHttpExceptions:true,headers:{'X-Ovh-Application':ak,'X-Ovh-Consumer':ck,'X-Ovh-Timestamp':String(timestamp),'X-Ovh-Signature':signature,'Accept':'application/json'}};
...
function normaliserTexteSmsGsmCerbereExpress20260827_(texte){
  const map={'🐺':'','☀️':'','🌤️':'','🌧️':'','⛈️':'','⚠️':'','—':'-','–':'-','’':"'",'‘':"'",'“':'"','”':'"','…':'...','ê':'e','ë':'e','â':'a','ä':'a','á':'a','í':'i','î':'i','ï':'i','ô':'o','ö':'o','ó':'o','ú':'u','û':'u','ü':'u','ÿ':'y','Ê':'E','Ë':'E','Â':'A','Ä':'A','Á':'A','Í':'I','Î':'I','Ï':'I','Ô':'O','Ö':'O','Ó':'O','Ú':'U','Û':'U','Ü':'U','Ÿ':'Y','œ':'oe','Œ':'OE'};
  let s=String(texte||'');
