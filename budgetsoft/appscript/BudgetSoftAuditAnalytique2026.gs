...
function auditTypesOperationsBudgetSoft2026(){
  ...
  ops.forEach(o=>{
    ...
    if(type&&type!=='depense'&&type!=='revenu'){typesBancairesNonDirectionnels++;if(problemes.length<200)problemes.push({id:o.id,date:o.date,libelle:o.libelle,montant,type,categorie,probleme:'type_bancaire_non_directionnel',attendu});}
    else if((type==='depense'||type==='revenu')&&attendu!==type){incoherencesSigneType++;if(problemes.length<200)problemes.push({id:o.id,date:o.date,libelle:o.libelle,montant,type,categorie,probleme:'signe_type',attendu});}
    if(categorie){const typeCat=typesCategories.get(categorie);if(!typeCat){categoriesInconnues++;if(problemes.length<200)problemes.push({id:o.id,date:o.date,libelle:o.libelle,montant,type,categorie,probleme:'categorie_inconnue'});}
      else if(typeCat==='tresorerie'){mouvementsTresorerie++;if(montant>0){tresorerieEntrante++;montantTresorerieEntrante+=montant;}else if(montant<0){tresorerieSortante++;montantTresorerieSortante+=Math.abs(montant);}}
      else if(typeCat==='epargne'){mouvementsEpargne++;if(montant>0){epargneEntrante++;montantEpargneEntrante+=montant;}else if(montant<0){epargneSortante++;montantEpargneSortante+=Math.abs(montant);}}
      else if((typeCat==='depense'||typeCat==='revenu')&&typeCat!==type){incoherencesCategorieType++;if(problemes.length<200)problemes.push({id:o.id,date:o.date,libelle:o.libelle,montant,type,categorie,probleme:'categorie_type',typeCategorie:typeCat});}}
  });
  const resultat={total:ops.length,incoherencesSigneType,incoherencesCategorieType,categoriesInconnues,typesBancairesNonDirectionnels,tresorerie:{operations:mouvementsTresorerie,entrantes:tresorerieEntrante,sortantes:tresorerieSortante,montantEntrant:Math.round(montantTresorerieEntrante*100)/100,montantSortant:Math.round(montantTresorerieSortante*100)/100,soldeNet:Math.round((montantTresorerieEntrante-montantTresorerieSortante)*100)/100},epargne:{operations:mouvementsEpargne,entrantes:epargneEntrante,sortantes:epargneSortante,montantEntrant:Math.round(montantEpargneEntrante*100)/100,montantSortant:Math.round(montantEpargneSortante*100)/100,soldeNet:Math.round((montantEpargneEntrante-montantEpargneSortante)*100)/100},problemes};
  console.log(JSON.stringify(resultat));return resultat;
}

function reparerDeuxiemeVagueCategorisation2026(){
  verifierInitialisation_();const ops=lireTable_('Operations');let modifiees=0;
  const detail={voyages:0,voitures:0,remboursements:0,cours:0,aides:0,concerts:0,telecom:0,achats:0,maison:0,sante:0,loisirs:0,courses:0,fraisPro:0,depensesDiverses:0,restaurants:0,correctionsCiblees:0,floaRevolving:0,virementsInternes55296:0,virementsInternesPatrick:0,aubagneRetroactif:0};
  ops.forEach(o=>{
    const texte=texteAuditAnalytique2026_(o),type=String(o.type||'').toLowerCase(),actuelle=String(o.categorie||'').trim(),montant=Number(o.montant||0),date=String(o.date||'').slice(0,10);let cible='';
    ...
    else if(type==='depense'&&/(VINTED|BIJOU BRIGITTE|MANGO|SHOWROOMPRIVE|PARFOIS|RELAY TOULOUSE)/.test(texte)&&!actuelle){cible='Achats personnels';detail.achats++;}
