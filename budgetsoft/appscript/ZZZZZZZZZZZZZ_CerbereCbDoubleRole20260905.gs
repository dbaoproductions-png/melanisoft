 ...
 * 2) Ces mêmes achats CB réduisent aussi le budget global de C2,
 *    cycle où leur débit différé sera réellement supporté en trésorerie.
 *
 ...
 */
const CERBERE_CB_DOUBLE_ROLE_VERSION='2026-09-05.8';
...
function calculerReportCbCycleSuivant20260905_(base){
  ...
  const diag={operations:ops.length,depenses:0,cbDetectees:0,avecDateAchat:0,dansFenetre:0,rejetsTechnique:0,avecDateComptable:0,dateComptableC2:0,sansDateComptable:0,fallbackSansDateComptable:0,rejetsHorsCycleComptable:0};
  const jour=d=>new Date(d.getFullYear(),d.getMonth(),d.getDate()).getTime();
  const aCb=jour(d11),z1=jour(d12),tn=jour(now),a2=jour(d21),z2=jour(d22);
  const debutCb=d11,impactCycle2=d21;
  ...
  (ops||[]).forEach(o=>{
    ...
    lignes.push({id:String(o&&o.id||''),categorie:String(o&&o.categorie||''),montant:Math.round(a*100)/100,dateAchat:Utilities.formatDate(da,Session.getScriptTimeZone(),'yyyy-MM-dd'),dateImpact:Utilities.formatDate(impact,Session.getScriptTimeZone(),'yyyy-MM-dd'),dateComptable:String(o&&o.date_comptable||''),carteFin:String(o&&o.carte_fin||''),libelle:String(o&&o.libelle_bancaire||o&&o.libelle||'')});
  });
...
function appliquerReportCbCycleSuivant20260905_(base){
  ...
  c.p1AvantReportCb=avant;c.reportCbCycle=report;c.cbDejaEngagee=report;c.p1Total=apres;c.p1Cible=apres;c.budgetPilotableActualise=apres;c.ret1=Math.round((apres-cons)*100)/100;c.surplusVsP0=Math.round((apres-p0)*100)/100;c.margeARepartir=Math.round((apres-rep)*100)/100;c.aRepartirDansPostes=c.margeARepartir;c.doctrineCb='CB : analytique par date achat dans C1 + impact global de trésorerie en C2 selon la date comptable, sans double imputation par catégorie.';
...
function chargerCerbereCockpit20260902(){
  ...
  return typeof avecContexteLectureBudgetSoft20260827_==='function'?avecContexteLectureBudgetSoft20260827_('cerbere-cockpit-cb-double-role-20260905-v8',executer):executer();
}
