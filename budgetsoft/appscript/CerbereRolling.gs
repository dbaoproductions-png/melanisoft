...
/** A/B strict : V33 autonome vs V33 alimenté par données préchargées. */
function auditerCandidatCerbereV33DonneesPrechargeesBudgetSoft20260911(){
  ...
  const statsBaseline=(typeof BUDGETSOFT_READ_CONTEXT_LAST_STATS_!=='undefined'&&BUDGETSOFT_READ_CONTEXT_LAST_STATS_)?BUDGETSOFT_READ_CONTEXT_LAST_STATS_:null;
  const tPre=Date.now();
  const precharge=avecContexteLectureBudgetSoft20260827_('audit-ab-v33-precharge-20260911',function(){return{operations:lireTable_('Operations'),charges:lireTable_('Charges_fixes'),comptes:lireTable_('Comptes'),parametres:lireTable_('Parametres'),objectifs:lireTablePlanCerbere_('Plan_Objectifs'),actions:lireFeuilleDynamiqueCerbereV3_('Plan_Actions'),evenements:lireTablePlanCerbere_('Plan_Evenements'),ajustements:lireAjustementsCerbereV33_()};});
  ...
  const originaux={lireTable:lireTable_,lirePlanTable:lireTablePlanCerbere_,lirePlanDyn:lireFeuilleDynamiqueCerbereV3_,lireAjustements:lireAjustementsCerbereV33_};
  let candidat=null,erreur=null,candidatMs=0,statsCandidat=null;
  try{
    lireTable_=function(nom){const n=String(nom||'');if(n==='Operations')return precharge.operations;if(n==='Charges_fixes')return precharge.charges;if(n==='Comptes')return precharge.comptes;if(n==='Parametres')return precharge.parametres;return originaux.lireTable.apply(this,arguments);};
    lireTablePlanCerbere_=function(nom){const n=String(nom||'');if(n==='Plan_Objectifs')return precharge.objectifs;if(n==='Plan_Evenements')return precharge.evenements;return originaux.lirePlanTable.apply(this,arguments);};
    lireFeuilleDynamiqueCerbereV3_=function(nom){if(String(nom||'')==='Plan_Actions')return precharge.actions;return originaux.lirePlanDyn.apply(this,arguments);};
    lireAjustementsCerbereV33_=function(){return precharge.ajustements;};
    const t1=Date.now();candidat=avecContexteLectureBudgetSoft20260827_('audit-ab-v33-candidat-precharge-20260911',function(){return chargerCerbereV33();});candidatMs=Date.now()-t1;
  ...
  }catch(e){erreur=String(e&&e.stack||e&&e.message||e);}finally{lireTable_=originaux.lireTable;lireTablePlanCerbere_=originaux.lirePlanTable;lireFeuilleDynamiqueCerbereV3_=originaux.lirePlanDyn;lireAjustementsCerbereV33_=originaux.lireAjustements;}
  const normaliser=typeof normaliserObjetProfilCerbere20260911_==='function'?normaliserObjetProfilCerbere20260911_:function(v){return v;},differ=typeof premieresDifferencesProfilCerbere20260911_==='function'?premieresDifferencesProfilCerbere20260911_:function(){return[];};
  const a=normaliser(baseline,''),b=normaliser(candidat,''),identique=!erreur&&JSON.stringify(a)===JSON.stringify(b),diffs=identique?[]:differ(a,b,20),gainMs=baselineMs-candidatMs,gainPct=baselineMs?Math.round(gainMs/baselineMs*1000)/10:null;
  const out={ok:identique&&!!(candidat&&candidat.ok!==false),version:'2026-09-11.7',lectureSeule:true,aucuneModification:true,perimetre:{compare:'chargerCerbereV33() autonome vs le même V33 recevant Operations, Charges_fixes, Comptes, Parametres et Plan déjà préchargés',reference:'exécution courante, même classeur, même instant métier',sourceVerite:'chargerCerbereV33 / CerbereV33.gs',cibleIntegration:'chemin snapshot uniquement ; chargeur autonome inchangé'},comparaison:{identiqueMetierStable:identique,differences:diffs,versionBaseline:String(baseline&&baseline.version||''),versionCandidat:String(candidat&&candidat.version||'')},temps:{baselineMs:baselineMs,prechargeMs:prechargeMs,candidatAvecDonneesDisponiblesMs:candidatMs,candidatTotalSiPrechargeDedieeMs:prechargeMs+candidatMs,gainMoteurMs:gainMs,gainMoteurPct:gainPct,dureeTotaleMs:Date.now()-tGlobal},precharge:{operations:(precharge.operations||[]).length,charges:(precharge.charges||[]).length,comptes:(precharge.comptes||[]).length,parametres:(precharge.parametres||[]).length,objectifs:(precharge.objectifs||[]).length,actions:(precharge.actions||[]).length,evenements:(precharge.evenements||[]).length},timingsV33:{baseline:baseline&&baseline.diagnostic&&baseline.diagnostic.timings||null,candidat:candidat&&candidat.diagnostic&&candidat.diagnostic.timings||null},lectures:{baseline:statsBaseline,precharge:statsPrecharge,candidat:statsCandidat},erreur:erreur,decision:identique?'CANDIDAT_V33_PRECHARGE_VALIDE_METIER_A_EVALUER_PERF':'CANDIDAT_V33_PRECHARGE_REFUSE_DIVERGENCE',doctrine:'A/B uniquement. Ne rien intégrer en production avant identité métier stricte, gain confirmé et passage des gardes du snapshot.'};
  console.log('[AUDIT A/B V33 données préchargées] '+JSON.stringify(out));return out;
}
