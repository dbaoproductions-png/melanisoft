...
function restaurerOperationsDepuisSauvegardeV2(nom){
  ...
  try{
    ...
    return{ok:true,sauvegardeSource:src.getName(),sauvegardeSecours:secours.getName(),apres};
...
function planifierSnapshotV23_(incoming,ops,compte){
  ...
  Object.keys(groupIn).forEach(k=>{
    const ins=groupIn[k],cands=(groupEx[k]||[]).slice().sort((a,b)=>{
      const ar=estRecurrenceV23_(a)?0:1,br=estRecurrenceV23_(b)?0:1;if(ar!==br)return ar-br;
    ...
    ins.forEach((x,j)=>{
      ...
      else if(possibles.length>1)ambigues.push({n:x.n,candidates:possibles.slice(0,5),raison:'plusieurs candidats résiduels'});
  ...
  matches.forEach(m=>{
    existants.forEach(o=>{
      if(used.has(String(o.id))||!estRecurrenceV23_(o))return;
  ...
  const orphelines=existants.filter(o=>{
    const d=dateJourV23_(o.date_comptable||o.date),src=String(o.source_bancaire||'').toLowerCase();
    if(!d||d<minDate||d>maxDate||used.has(String(o.id)))return false;
    return estRecurrenceV23_(o)||src==='flux';
  });
...
function analyserFluxBancaireAvantImportV2(lignes,compte){
  const recues=(lignes||[]).length,incoming=preparerFluxV23_(lignes,compte),rejetes=Math.max(0,recues-incoming.length),ctl=controlerLotBancaire_(incoming,null),ops=lireOperationsBancaires_().map(enrichirDepuisCommentaireBanque_),p=planifierSnapshotV23_(incoming,ops,compte);
...
function importerFluxBancaireControleV2(lignes,compte){
  ...
  try{
    ...
    const out=ops.filter(o=>!supprimer.has(String(o.id))).map(o=>{
      ...
      return Object.assign({},o,{date:n.date,date_comptable:n.date_comptable,date_achat:n.date_achat,libelle_bancaire:n.libelle_bancaire,marchand_normalise:n.marchand_normalise,carte_fin:n.carte_fin,source_bancaire:'flux',statut_bancaire:'provisoire',commentaire:comment,modifie_le:new Date()});
    });
