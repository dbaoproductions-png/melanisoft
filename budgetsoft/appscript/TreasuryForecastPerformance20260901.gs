...
function chargerTresoreriePrevisionnelleRapide20260901(dateCible){
  return chargerTresoreriePrevisionnelleRapideInterne20260903_(dateCible,null,false);
}

/**
 * Variante dédiée au cockpit Cerbère : la part Cerbère du prochain débit CB est
...
function chargerTresoreriePrevisionnelleRapideInterne20260903_(dateCible,partCerbereFournie,interdireRappelCerbere){
  const t0=Date.now();
  return avecContexteLectureBudgetSoft20260827_('tresorerie_previsionnelle_rapide_20260903',function(){
    ...
    const socle={ok:true,version:TREASURY_FORECAST_PERF_20260901_VERSION,soldeReel:soldeReel,comptes:comptes.map(c=>({id:c.id,nom:c.nom,soldeReel:c.soldeReel,dateSolde:c.dateSolde,sourceSolde:c.sourceSolde}))};
    ...
    const evs=occurrencesEvenementsTresorerie_(evenements,hard,reference,cible,comptes).filter(x=>evenementEffectifTresorerie20260831_(x.sourceId,evenements));
    ...
    let debitCb=null;
    if(prochainDebit<=cible){
      if(interdireRappelCerbere){
        const partFinMois=estimationQueueCbFinMoisTresorerie20260901_(ops,reference,prochainDebit);
        const residuel=arrondiTresorerie_(Math.max(0,Number(partCerbereFournie||0)+Number(partFinMois||0)));
        if(residuel>0)debitCb={id:'debit_cb_estime:'+prochainDebit.getTime(),source:'debit_cb_estime',sourceId:'cb:'+prochainDebit.getFullYear()+'-'+(prochainDebit.getMonth()+1),date:prochainDebit.toISOString(),libelle:'Complément estimé du débit CB différé',categorie:'Carte à débit différé',compte:'',montantSigne:-residuel,certitude:'estime',preuve:'Reste Cerbère déjà calculé + estimation marginale des jours 28-fin de mois',dateConventionnelle:false,partCerbere:arrondiTresorerie_(partCerbereFournie),partFinMois:arrondiTresorerie_(partFinMois),moteurCerbere:'fourni-par-cockpit'};
      }else debitCb=estimationDebitCbDiffereTresorerie20260901V2_(ops,reference,cible);
    }
    ...
    r.version=TREASURY_FORECAST_PERF_20260901_VERSION;r.debitCbEstime=debitCb||null;
    r.performance=Object.assign({},r.performance||{},{fastPath:true,dureeMs:Date.now()-t0,cerbereAppelee:prochainDebit<=cible&&!interdireRappelCerbere,partCerbereFournie:!!interdireRappelCerbere,operations:(ops||[]).length,chargesFixes:(charges||[]).length,evenements:(evenements||[]).length,actions:(actions||[]).length,lignesFinales:lignes.length});
    r.diagnostic20260831=r.diagnostic20260831||{};r.diagnostic20260831.cheminRapide=true;
...
function auditerPerformanceTresorerie20260901(){
  const reference=new Date();const y=reference.getFullYear(),m=reference.getMonth();
  ...
  dates.push(Utilities.formatDate(new Date(y,m+1,3),Session.getScriptTimeZone(),'yyyy-MM-dd'));
  const essais=[];
  dates.forEach(date=>{let t=Date.now(),ancien=chargerTresoreriePrevisionnelle20260901(date),ancienMs=Date.now()-t;t=Date.now();let rapide=chargerTresoreriePrevisionnelleRapide20260901(date),rapideMs=Date.now()-t;essais.push({date,ancienMs,rapideMs,gainPct:ancienMs?Math.round((1-rapideMs/ancienMs)*1000)/10:null,ancien:ancien.soldePrevisionnel,rapide:rapide.soldePrevisionnel,ecart:arrondiTresorerie_(Number(rapide.soldePrevisionnel||0)-Number(ancien.soldePrevisionnel||0)),identique:Math.abs(Number(rapide.soldePrevisionnel||0)-Number(ancien.soldePrevisionnel||0))<.011});});
