function analyserImportChargesFixes(texte) {
  ...
  const resultat = lignes.map((ligne, index) => {
    ...
    if (!jour || jour < 1 || jour > 31) erreurs.push('Jour invalide');
    ...
    const doublon = compteTrouve ? chargesParCle[cleImport_(libelle, compteTrouve.id)] || chargesParCle[cleImport_(libelle, compteTrouve.nom)] : null;
    return {
      numero: index + 1,
      libelle,
      montant,
      ...
      categorie,
      frequence: String(ligne.frequence || 'Mensuel').trim(),
      erreurs,
      avertissements,
  ...
  return {
    ...
    nouvellesCategories: [...new Set(resultat.flatMap(l => l.avertissements).filter(a => a.indexOf('Nouvelle catégorie : ') === 0).map(a => a.replace('Nouvelle catégorie : ', '')))]
  };
}
...
function importerChargesFixes(lignes, creerCategories) {
  ...
  lignes.forEach((ligne, index) => {
    try {
      if (!ligne.valide || ligne.action === 'ignorer') { bilan.ignorees++; return; }
      const categorie = String(ligne.categorie || '').trim();
      ...
      if (ligne.doublon_id && ligne.action === 'mettre_a_jour') bilan.misesAJour++; else bilan.creees++;
    } catch (e) { bilan.erreurs.push('Ligne ' + (index + 1) + ' : ' + e.message); }
  });
...
function normaliserEnteteImport_(v) {
  ...
  const alias = { libelle_charge:'libelle', montant_mensuel:'montant', jour_du_mois:'jour', jour_execution:'jour', nom_du_compte:'compte' };
  return alias[n] || n;
}
...
function normaliserTexteImport_(v) { return String(v || '').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); }
function indexerParNom_(lignes, champ) { return Object.fromEntries(lignes.map(l => [normaliserTexteImport_(l[champ]), l])); }
function cleImport_(libelle, compte) { return normaliserTexteImport_(libelle) + '|' + normaliserTexteImport_(compte); }
function journaliserImport_(bilan) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let feuille = ss.getSheetByName('Journal');
  if (!feuille) { feuille = ss.insertSheet('Journal'); feuille.appendRow(['date', 'action', 'details', 'utilisateur']); feuille.setFrozenRows(1); feuille.getRange(1,1,1,4).setFontWeight('bold').setBackground('#147d64').setFontColor('#ffffff'); }
  feuille.appendRow([new Date(), 'Import charges fixes', bilan.creees + ' créée(s), ' + bilan.misesAJour + ' mise(s) à jour, ' + bilan.ignorees + ' ignorée(s)', Session.getActiveUser().getEmail() || 'Utilisateur']);
}