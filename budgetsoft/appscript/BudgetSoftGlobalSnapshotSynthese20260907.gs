const BUDGETSOFT_GLOBAL_SYNTHESE_20260907_VERSION='2026-09-07.8';
...
function reconstruireSnapshotGlobalSyntheseBudgetSoft20260907(origine){
  ...
  try{
    const construire=function(){
      ...
      // Garde permanente : une trajectoire qui atteint le cycle suivant doit conserver
      // tous les débits CB jusqu'à la fin bancaire de ce cycle. Elle bloque notamment
      // toute régression vers l'ancien comportement « premier débit seulement ».
      const gardeCbMultiCycle=prendre('gardeCbMultiCycle',()=>{
        if(typeof auditerGardeCbMultiCycleBudgetSoft20260908!=='function')return{ok:false,erreur:'Garde CB multi-cycle absente.'};
        return auditerGardeCbMultiCycleBudgetSoft20260908(finSuivant);
      });
