 ...
 */
const BUDGETSOFT_UNIFIED_TREASURY_VERSION='2026-09-07.2';

function jourTresorerieUnifiee20260907_(v){const d=v instanceof Date?new Date(v):new Date(v||0);if(isNaN(d.getTime()))return '';return Utilities.formatDate(d,Session.getScriptTimeZone(),'yyyy-MM-dd');}
...
function auditerUniteModulesTresorerieBudgetSoft20260907_(modules){
  ...
  const valeurs={
    comptes:Number(c&&c.synthese&&c.synthese.disponible),
    moteur:Number(p&&p.soldeReel),
    dashboard:Number(d&&d.courtTerme&&d.courtTerme.soldeBancaire),
    cerbere:Number(cer&&cer.reel&&cer.reel.soldeBancaire)
  };
  ...
  const versionMoteur=String(p&&p.version||''),moteurDoctrinal=/^2026-09-01\./.test(versionMoteur);
  return{ok:presentes.length>=3&&ecart!=null&&Math.abs(ecart)<=.01&&moteurDoctrinal,version:BUDGETSOFT_UNIFIED_TREASURY_VERSION,valeurs,ecartSoldeReel:ecart,versionMoteur,moteurDoctrinal,composants:presentes.map(([k])=>k)};
}
